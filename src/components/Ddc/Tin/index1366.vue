<template>
  <div class="g-title-smaller" style="min-width: 1145px; overflow-x: scroll;">
    <div class="top-tips">
      <i class="el-icon-warning g-color-warning" style="margin-right: 10px;" />
      <span class="g-color-gray">{{ tip }}</span>
    </div>

    <div class="g-flex" style="margin: 0 8px;">
      <div class="marginright8" style="width:calc(100% - 348px);">
        <div class="g-flex">
          <!-- 用户信息 -->
          <div class="index-frame g-flex g-aligncenter marginright8" style="width:220px;">
            <el-image class="user-avator" src="https://fuss10.elemecdn.com/e/5d/4a731a90594a4af544c0c25941171jpeg.jpeg" fit="cover" />
            <div style="margin-left:15px;">
              <div class="g-color-title g-strong g-title-mini">余晨望</div>
              <div class="user-post g-margin-tb">
                <img src="~@/assets/images/workbench/icon_position.png" style="margin-right: 10px;height:20px; width:20px;" alt="">
                客服专员
              </div>
              <div class="g-color-tag">大道成物流 | 客服部</div>
            </div>
          </div>
          <!-- 便捷导航 -->
          <div class="index-frame" style="width:calc(100% - 220px);">
            <div class="g-color-bigtitle g-strong g-title-mini">便捷导航</div>
            <div class="g-flex g-aligncenter g-justifyaround" style="margin-top: 10px;">
              <div class="g-flex g-aligncenter g-flexdirection width10">
                <img class="nav-icon" src="~@/assets/images/workbench/icon_addcompany.png" alt="">
                <div class="g-color-title g-strong margintop5">新增企业</div>
              </div>
              <div class="g-flex g-aligncenter g-flexdirection width10">
                <img class="nav-icon" src="~@/assets/images/workbench/icon_report.png" alt="">
                <div class="g-color-title g-strong margintop5">数据上报</div>
              </div>
              <div class="g-flex g-aligncenter g-flexdirection width10">
                <img class="nav-icon" src="~@/assets/images/workbench/icon_order.png" alt="">
                <div class="g-color-title g-strong margintop5">发布货源</div>
              </div>
              <div class="g-flex g-aligncenter g-flexdirection width10">
                <img class="nav-icon" src="~@/assets/images/workbench/icon_addwaybill.png" alt="">
                <div class="g-color-title g-strong margintop5">新增运单</div>
              </div>
              <div class="g-flex g-aligncenter g-flexdirection width10">
                <img class="nav-icon" src="~@/assets/images/workbench/icon_waybillmanage.png" alt="">
                <div class="g-color-title g-strong margintop5">运单管理</div>
              </div>
              <div class="g-flex g-aligncenter g-flexdirection width10">
                <img class="nav-icon" src="~@/assets/images/workbench/icon_billing.png" alt="">
                <div class="g-color-title g-strong margintop5">开具发票</div>
              </div>
              <div class="g-flex g-aligncenter g-flexdirection width10">
                <img class="nav-icon" src="~@/assets/images/workbench/icon_invoicelist.png" alt="">
                <div class="g-color-title g-strong margintop5">发票列表</div>
              </div>
              <div class="g-flex g-aligncenter g-flexdirection width10">
                <img class="nav-icon" src="~@/assets/images/workbench/icon_waybillcp.png" alt="">
                <div class="g-color-title g-strong margintop5">运单投诉</div>
              </div>
            </div>
          </div>
        </div>
        <div class="g-flex margintop8">
          <!-- 运单 -->
          <div class="index-frame marginright8" style="height: 280px; width:calc(100% - 432px);">
            <!-- <div class="g-color-bigtitle g-strong g-title-mini">运单</div> -->
            <div class="g-flex g-justifybetween" style="height:100%;">
              <div style="width:150px; margin: 25px 10px 0;">
                <div class="g-flex g-aligncenter">
                  <img class="marginright15" src="~@/assets/images/workbench/icon_waybillnum.png" alt="">
                  <div>
                    <div class="g-color-title">运单总数</div>
                    <div class="g-color-tag"><span class="g-color-title g-strong g-title-big font-Bahnschrif">23436</span> 单</div>
                  </div>
                </div>
                <div class="g-flex g-aligncenter" style="margin-top:40px;">
                  <img class="marginright15" src="~@/assets/images/workbench/icon_waybillnum.png" alt="">
                  <div>
                    <div class="g-color-title">今日运单数</div>
                    <div class="g-color-tag"><span class="g-color-title g-strong g-title-big font-Bahnschrif">436</span> 单</div>
                  </div>
                </div>
                <img class="waybill-bg" src="~@/assets/images/workbench/icon_waybillbg.png" alt="">
              </div>
              <div class="g-flex g-flexdirection g-justifyaround" style="width: 65%; margin-right:10px;">
                <div>
                  <div class="g-color-title">
                    <span class="g-color-blue marginright5">●</span>今日已装货
                    <span class="g-color-title g-strong g-title-mini font-Bahnschrif">334</span>单
                  </div>
                  <el-progress :stroke-width="4" :percentage="70" :show-text="false" />
                </div>
                <div>
                  <div class="g-color-title">
                    <span class="g-color-blue marginright5">●</span>今日已卸货
                    <span class="g-color-title g-strong g-title-mini font-Bahnschrif">200</span>单
                  </div>
                  <el-progress :stroke-width="4" :percentage="60" :show-text="false" />
                </div>
                <div>
                  <div class="g-color-title">
                    <span class="g-color-blue marginright5">●</span>今日已复核
                    <span class="g-color-title g-strong g-title-mini font-Bahnschrif">160</span>单
                  </div>
                  <el-progress :stroke-width="4" :percentage="50" :show-text="false" />
                </div>
                <div>
                  <div class="g-color-title">
                    <span class="g-color-blue marginright5">●</span>今日已结算
                    <span class="g-color-title g-strong g-title-mini font-Bahnschrif">120</span>单
                  </div>
                  <el-progress :stroke-width="4" :percentage="40" :show-text="false" />
                </div>
                <div>
                  <div class="g-color-title">
                    <span class="g-color-blue marginright5">●</span>今日已打款
                    <span class="g-color-title g-strong g-title-mini font-Bahnschrif">100</span>单
                  </div>
                  <el-progress :stroke-width="4" :percentage="30" :show-text="false" />
                </div>

              </div>
            </div>
          </div>
          <div style="width:432px;">
            <!-- 货单 -->
            <div class="index-frame" style="height: 136px;">
              <!-- <div class="g-color-bigtitle g-strong g-title-mini">货单</div> -->
              <div class="g-flex g-aligncenter" style="height: 100%;">
                <div class="g-flex g-flexdirection g-justifyaround" style="margin-right: 30px; margin-left:10px;height: 100%;">
                  <div>
                    <div class="g-color-title">货单总数</div>
                    <div class="g-color-tag"><span class="g-color-title g-strong g-title-big font-Bahnschrif">23436</span> 单</div>
                  </div>
                  <div>
                    <div class="g-color-title">今日货单</div>
                    <div class="g-color-tag"><span class="g-color-title g-strong g-title-big font-Bahnschrif">436</span> 单</div>
                  </div>
                </div>
                <div class="order-frame" style="margin-right: 8px;">
                  <img class="order-img" src="~@/assets/images/workbench/icon_publishorder.png" alt="">
                  <div class="g-color-title margintop8">发布中</div>
                  <div class="g-color-tag"><span class="g-color-title g-strong g-title-mini font-Bahnschrif">436</span> 单</div>
                </div>
                <div class="order-frame">
                  <img class="order-img" src="~@/assets/images/workbench/icon_closeorder.png" alt="">
                  <div class="g-color-title margintop8">已关闭</div>
                  <div class="g-color-tag"><span class="g-color-title g-strong g-title-mini font-Bahnschrif">436</span> 单</div>
                </div>
              </div>
            </div>
            <!-- 发票 -->
            <div class="index-frame margintop8" style="height: 136px;">
              <!-- <div class="g-color-bigtitle g-strong g-title-mini">发票</div> -->
              <div class="g-flex g-aligncenter" style="height: 100%;">
                <div class="g-flex g-flexdirection g-justifyaround" style="margin-right: 30px; margin-left:10px; height: 100%;">
                  <div>
                    <div class="g-color-title">发票总数</div>
                    <div class="g-color-tag"><span class="g-color-title g-strong g-title-big font-Bahnschrif">23436</span> 单</div>
                  </div>
                  <div>
                    <div class="g-color-title">今日发票</div>
                    <div class="g-color-tag"><span class="g-color-title g-strong g-title-big font-Bahnschrif">436</span> 单</div>
                  </div>
                </div>
                <div class="order-frame" style="margin-right: 8px;">
                  <img class="order-img" src="~@/assets/images/workbench/icon_ask.png" alt="">
                  <div class="g-color-title margintop8">今日已索票</div>
                  <div class="g-color-tag"><span class="g-color-title g-strong g-title-mini font-Bahnschrif">436</span> 单</div>
                </div>
                <div class="order-frame" style="margin-right: 8px;">
                  <img class="order-img" src="~@/assets/images/workbench/icon_examine.png" alt="">
                  <div class="g-color-title margintop8">今日已审核</div>
                  <div class="g-color-tag"><span class="g-color-title g-strong g-title-mini font-Bahnschrif">436</span> 单</div>
                </div>
                <div class="order-frame">
                  <img class="order-img" src="~@/assets/images/workbench/icon_onbilling.png" alt="">
                  <div class="g-color-title margintop8">已开票</div>
                  <div class="g-color-tag"><span class="g-color-title g-strong g-title-mini font-Bahnschrif">436</span> 单</div>
                </div>
              </div>
            </div>
          </div>
        </div>
        <div>
          <!-- 平台概览 -->
          <div class="index-frame g-flex margintop8">
            <!-- <div class="g-color-bigtitle g-strong g-title-mini">平台概览</div> -->
            <div class="g-flex g-aligncenter margintop5" style="width:100%;">
              <div class="g-flex g-aligncenter g-flexdirection" style="width:25%">
                <div class="g-flex g-aligncenter">
                  <el-progress type="circle" :percentage="80" :stroke-width="10" :width="100" color="#409EFF" :show-text="false" />
                  <div class="platform-num g-flex g-aligncenter g-flexdirection">
                    <div class="g-color-title g-strong g-title-big font-Bahnschrif">4514</div>
                    <div class="g-color-title" style="white-space: nowrap;">平台货主</div>
                  </div>
                </div>
                <div style="margin-top:5px;"><span class="g-color-blue marginright5">●</span>活跃货主</div>
                <div class="g-flex g-aligncenter g-justifyaround" style="width: 90%">
                  <div>
                    <div class="g-color-title g-strong g-title-small font-Bahnschrif">260</div>
                    <div class="g-color-tag">企业数</div>
                  </div>
                  <div>
                    <div class="g-color-title g-strong g-title-small font-Bahnschrif">300</div>
                    <div class="g-color-tag">账号数</div>
                  </div>
                  <div>
                    <div class="g-color-title g-strong g-title-small font-Bahnschrif">160</div>
                    <div class="g-color-tag">活跃企业</div>
                  </div>
                </div>
              </div>
              <div class="g-flex g-aligncenter g-flexdirection" style="width:25%">
                <div class="g-flex g-aligncenter">
                  <el-progress type="circle" :percentage="70" :stroke-width="10" :width="100" color="#40CBFF" :show-text="false" />
                  <div class="platform-num g-flex g-aligncenter g-flexdirection">
                    <div class="g-color-title g-strong g-title-big font-Bahnschrif">15067</div>
                    <div class="g-color-title" style="white-space: nowrap;">平台司机</div>
                  </div>
                </div>
                <div style="margin-top:5px;"><span class="g-color-blue marginright5">●</span>活跃司机</div>
                <div class="g-flex g-aligncenter g-justifyaround" style="width: 90%">
                  <div>
                    <div class="g-color-title g-strong g-title-small font-Bahnschrif">260</div>
                    <div class="g-color-tag">零散司机</div>
                  </div>
                  <div>
                    <div class="g-color-title g-strong g-title-small font-Bahnschrif">300</div>
                    <div class="g-color-tag">聘用司机</div>
                  </div>
                  <div>
                    <div class="g-color-title g-strong g-title-small font-Bahnschrif">15067</div>
                    <div class="g-color-tag">活跃司机</div>
                  </div>
                </div>
              </div>
              <div class="g-flex g-aligncenter g-flexdirection" style="width:25%">
                <div class="g-flex g-aligncenter">
                  <el-progress type="circle" :percentage="30" :stroke-width="10" :width="100" color="#67C23A" :show-text="false" />
                  <div class="platform-num g-flex g-aligncenter g-flexdirection">
                    <div class="g-color-title g-strong g-title-big font-Bahnschrif">15099</div>
                    <div class="g-color-title" style="white-space: nowrap;">平台车辆</div>
                  </div>
                </div>
                <div style="margin-top:5px;"><span class="g-color-success marginright5">●</span>活跃车辆</div>
                <div class="g-flex g-aligncenter g-justifyaround" style="width: 90%">
                  <div>
                    <div class="g-color-title g-strong g-title-small font-Bahnschrif">260</div>
                    <div class="g-color-tag">活跃车辆</div>
                  </div>
                </div>
              </div>
              <div class="g-flex g-aligncenter g-flexdirection" style="width:25%">
                <div class="g-flex g-aligncenter">
                  <el-progress type="circle" :percentage="60" :stroke-width="10" :width="100" color="#FFBB00" :show-text="false" />
                  <div class="platform-num g-flex g-aligncenter g-flexdirection">
                    <div class="g-color-title g-strong g-title-big font-Bahnschrif">13067</div>
                    <div class="g-color-title" style="white-space: nowrap;">平台调度者</div>
                  </div>
                </div>
                <div style="margin-top:5px;"><span class="g-color-warning marginright5">●</span>活跃调度者</div>
                <div class="g-flex g-aligncenter g-justifyaround" style="width: 90%">
                  <div>
                    <div class="g-color-title g-strong g-title-small font-Bahnschrif">160</div>
                    <div class="g-color-tag">活跃调度者</div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
      <div style="width: 348px;">
        <!-- 公告 -->
        <div class="notice-frame" style="height:107px; width: 348px;">
          <div class="notice-img">
            <div class="notice-content">
              <div class="g-color-title g-strong g-title-mini">系统公告</div>
              <div class="g-color-title notice-tip">{{ tip }}</div>
            </div>
          </div>
          <div class="notice-bottom" />
        </div>
        <!-- 动态 -->
        <div class="index-frame margintop8" style="height:468px;">
          <!-- <div class="g-color-bigtitle g-strong g-title-mini">动态</div> -->
          <el-row class="trend-tab">
            <el-col :span="8">
              <div :class="activeName === '1' ? 'trend-onbottom': 'trend-bottom'" @click="handleClick('1')">运单动态</div>
            </el-col>
            <el-col :span="8">
              <div :class="activeName === '2' ? 'trend-onbottom': 'trend-bottom'" @click="handleClick('2')">货单动态</div>
            </el-col>
            <el-col :span="8">
              <div :class="activeName === '3' ? 'trend-onbottom': 'trend-bottom'" @click="handleClick('3')">发票动态</div>
            </el-col>
          </el-row>
          <!-- 运单动态 -->
          <div v-if="activeName === '1'" class="trend-frame">
            <div class="trend-content g-flex g-aligncenter">
              <img src="~@/assets/images/workbench/icon_addwaybill1.png" alt="">
              <div style="margin-left: 12px;">
                <div class="g-color-tag">2分钟前 2021-01-28 09:49:23</div>
                <div class="g-color-title g-strong margintop5">余晨望 创建订单（1092838191201）</div>
              </div>
            </div>
            <div class="trend-content g-flex g-aligncenter">
              <div class="trend-line" />
              <img src="~@/assets/images/workbench/icon_closewaybill.png" alt="">
              <div style="margin-left: 12px;">
                <div class="g-color-tag">2分钟前 2021-01-28 09:49:23</div>
                <div class="g-color-title g-strong margintop5">余晨望 取消订单（1092838191201）</div>
              </div>
            </div>
            <div class="trend-content g-flex g-aligncenter">
              <div class="trend-line" />
              <img src="~@/assets/images/workbench/icon_load.png" alt="">
              <div style="margin-left: 12px;">
                <div class="g-color-tag">2分钟前 2021-01-28 09:49:23</div>
                <div class="g-color-title g-strong margintop5">余晨望 运单装货（1092838191201）</div>
              </div>
            </div>
            <div v-for="(item, index) in 10" :key="index" class="trend-content g-flex g-aligncenter">
              <div class="trend-line" />
              <img src="~@/assets/images/workbench/icon_unload.png" alt="">
              <div style="margin-left: 12px;">
                <div class="g-color-tag">2分钟前 2021-01-28 09:49:23</div>
                <div class="g-color-title g-strong margintop5">余晨望 运单卸货（1092838191201）</div>
              </div>
            </div>
          </div>
          <!-- 货单动态 -->
          <div v-if="activeName === '2'" class="trend-frame">
            <div class="trend-oncontent g-flex g-aligncenter">
              <img src="~@/assets/images/workbench/icon_publishorder1.png" alt="">
              <div style="margin-left: 12px;">
                <div class="g-color-blue">2分钟前 2021-01-28 09:49:23</div>
                <div class="g-color-blue g-strong margintop5">余晨望 发布货源（1092838191201）</div>
              </div>
            </div>
            <div class="trend-content g-flex g-aligncenter">
              <div class="trend-line" />
              <img src="~@/assets/images/workbench/icon_closeorder1.png" alt="">
              <div style="margin-left: 12px;">
                <div class="g-color-tag">2分钟前 2021-01-28 09:49:23</div>
                <div class="g-color-title g-strong margintop5">余晨望 关闭货源（1092838191201）</div>
              </div>
            </div>
            <div v-for="(item, index) in 10" :key="index" class="trend-content g-flex g-aligncenter">
              <div class="trend-line" />
              <img src="~@/assets/images/workbench/icon_publishorder1.png" alt="">
              <div style="margin-left: 12px;">
                <div class="g-color-tag">2分钟前 2021-01-28 09:49:23</div>
                <div class="g-color-title g-strong margintop5">余晨望 发布货源（1092838191201）</div>
              </div>
            </div>
          </div>
          <!-- 发票动态 -->
          <div v-if="activeName === '3'" class="trend-frame">
            <div class="trend-content g-flex g-aligncenter">
              <img src="~@/assets/images/workbench/icon_applyinvoice.png" alt="">
              <div style="margin-left: 12px;">
                <div class="g-color-tag">2分钟前 2021-01-28 09:49:23</div>
                <div class="g-color-title g-strong margintop5">余晨望 货主申请开票（1092838191201）</div>
              </div>
            </div>
            <div v-for="(item, index) in 10" :key="index" class="trend-content g-flex g-aligncenter">
              <div class="trend-line" />
              <img src="~@/assets/images/workbench/icon_applyinvoice.png" alt="">
              <div style="margin-left: 12px;">
                <div class="g-color-tag">2分钟前 2021-01-28 09:49:23</div>
                <div class="g-color-title g-strong margintop5">余晨望 货主申请开票（1092838191201）</div>
              </div>
            </div>
          </div>
          <div class="notice-bottom" />
        </div>
      </div>
    </div>

  </div>
</template>

<script>
// import { color } from 'echarts';
export default {
  name: 'Index',
  data() {
    return {
      // 版本号
      version: '1.2.4.0',
      activeName: '1',
      customColor: 'linear-gradient(180deg, #409EFF 0%, #006FE2 100%)',
      tip: '亲爱的用户，2021年4月22日（周四晚）21:30至23:00,银行系统升级，这期可能存在部分运单支付后较长时间处于“支付中”，为了您能正常支付运单，建议您避开此时段操作支付感谢您的支持！'
    };
  },
  methods: {
    goTarget(href) {
      window.open(href, '_blank');
    },
    handleClick(tab) {
      this.activeName = tab;
    }
  }
};
</script>

<style scoped lang="scss">
.font-Bahnschrif{
  font-family: "Bahnschrif";
}
.index-frame{
  padding: 8px;
  background: #fff;
  border-radius: 4px;
}
.margintop8{
  margin-top: 8px;
}
.margintop15{
  margin-top: 15px;
}
.margintop5{
  margin-top: 5px;
}
.marginright15{
  margin-right: 15px;
}
.marginright8{
  margin-right: 8px;
}
.marginright5{
  margin-right: 5px;
}
.margin15{
  margin: 0 15px 15px;
}
.width10{
  width: 10%;
}
.nav-icon{
  height: 39px;
  width: 39px;
}
.top-tips{
  background: #FFFBE6;
  border: 1px solid #FFE58F;
  border-radius: 2px;
  margin: 0 8px 8px;
  padding: 9px 12px;
  overflow: hidden;
  text-overflow: ellipsis;
}
// 用户信息
.user-avator{
  width: 72px;
  height: 72px;
  border-radius: 50%;
  border: 10px solid #F9F9F9;
}
.user-post{
  display: flex;
  padding: 0 10px;
  height: 20px;
  background: #EBF4FD;
  border-radius: 11px;
  font-size: 12px;
  font-weight: 500;
  line-height: 20px;
  color: #409EFF;
}
// 系统公告
.notice-frame{
  background: #fff;
  border-radius: 4px;
}
.notice-bottom{
  width: 100%;
  height: 44px;
  background: linear-gradient(180deg, rgba(255,255,255,0) 0%, #FFFFFF 100%);
  border-radius: 4px;
  position: relative;
  bottom: 44px;
}
.notice-img{
  background: url('~@/assets/images/workbench/icon_notice.png') no-repeat;
  background-size: 164px 107px;
  height: 107px;
}
.notice-content{
  margin-top: 10px;
  float: right;
  width: 68%;
}
.notice-tip{
  margin-top: 10px;
  margin-right: 10px;
  // font-family: PingFang SC;
  height: 65px;
  overflow: hidden; //超出隐藏
}
// 运单
.waybill-bg{
  height: 100px;
  width: 148px;
  position: relative;
  top: 10px;
  left: -18px;
}
::v-deep .el-progress-bar__inner {
  background: linear-gradient(270deg, #409EFF 0%, rgba(64, 158, 255, 0.51) 100%);
}
// 货单
.order-frame{
  width: 88px;
  height: 88px;
  background: #F3F3F3;
  border-radius: 9px;
  padding: 8px;
}
.order-img{
  height: 22px;
  width: 22px;
}
// 平台概览
.platform-num{
  position: relative;
  bottom: 0;
  left: -50px;
  width: 0;
}
// 动态
.trend-tab{
  cursor: pointer;
  height: 30px;
  background: #F3F3F3;
  border-radius: 16px;
  overflow: hidden;
  margin: 10px 20px;
}
.trend-onbottom{
  height: 32px;
  background: #409EFF;
  color: #fff;
  font-weight: bold;
  display: flex;
  align-items: center;
  justify-content: center;
}
.trend-bottom{
  height: 32px;
  color: #9FA2B5;
  font-weight: bold;
  display: flex;
  align-items: center;
  justify-content: center;
}
.trend-frame{
  height: 410px;
  overflow: hidden;
}
.trend-line{
  position: relative;
  bottom: 30px;
  left: 12px;
  width: 0px;
  height: 24px;
  border: 1px dashed #E6E6E6;
}
.trend-oncontent{
  height: 60px;
  padding: 10px 15px;
  background: #F9FBFC;
}
.trend-content{
  height: 60px;
  padding: 10px 10px;
}

.home {
  blockquote {
    padding: 10px 20px;
    margin: 0 0 20px;
    font-size: 17.5px;
    border-left: 5px solid #eee;
  }
  hr {
    margin-top: 20px;
    margin-bottom: 20px;
    border: 0;
    border-top: 1px solid #eee;
  }
  .col-item {
    margin-bottom: 20px;
  }

  ul {
    padding: 0;
    margin: 0;
  }

  font-family: "Bahnschrif";
  font-size: 13px;
  color: #676a6c;
  overflow-x: hidden;

  ul {
    list-style-type: none;
  }

  h4 {
    margin-top: 0px;
  }

  h2 {
    margin-top: 10px;
    font-size: 26px;
    font-weight: 100;
  }

  p {
    margin-top: 10px;

    b {
      font-weight: 700;
    }
  }

  .update-log {
    ol {
      display: block;
      list-style-type: decimal;
      margin-block-start: 1em;
      margin-block-end: 1em;
      margin-inline-start: 0;
      margin-inline-end: 0;
      padding-inline-start: 40px;
    }
  }
}
</style>

