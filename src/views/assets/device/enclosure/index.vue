  ...
  <div class="device-info ly-flex">
    <div class="device-info-left">
        ...
        <div class="device-info-list-box">
          ...
          >
            <ul
              ...
              class="device-info-list infinite-list own-device-scroll-box"
            >
              <li
                ...
                :class="{
                  active: activeCard === item.typeCode + item.factoryOnlyCode,
                }"
              ...
              >
                ...
                >
                  <p class="label ly-flex ly-flex-align-center">
                    <el-checkbox
                      :label="item.typeCode + ',' + item.factoryOnlyCode"
                      :disabled="item.activationFlag !== 1"
                      @click.native.stop="
                        handleCheckActive(
                          item.activationFlag,
                          item.typeCode + item.factoryOnlyCode
                        )
                      "
                    />
                  ...
                  <el-tooltip
                    class="item"
                    effect="dark"
                    :content="computeOfflineTime(item.data && item.data.hbTime)"
                    placement="top-start"
                ...
                <div class="info-groud ly-flex ly-flex-align-center">
                  <div
                    v-if="item.data && item.data.electQuantity"
                    class="info-groud-item"
                  >
                    <p class="label">设备电量</p>
                    <div class="ly-flex ly-flex-align-center">
                      <img
                        v-if="item.data.electQuantity > 90"
                        class="mr5"
                        :src="
                          require('@/assets/images/device/dl3' +
                            (activeCard === item.typeCode + item.factoryOnlyCode
                              ? '_hov'
                              : '') +
                            '.png')
                        "
                      />
                      <img
                        v-else-if="item.data.electQuantity > 60"
                        class="mr5"
                        :src="
                          require('@/assets/images/device/dl2' +
                            (activeCard === item.typeCode + item.factoryOnlyCode
                              ? '_hov'
                              : '') +
                            '.png')
                      ...
                      />
                      <img
                        v-else-if="item.data.electQuantity > 10"
                        class="mr5"
                        :src="
                          require('@/assets/images/device/dl1' +
                            (activeCard === item.typeCode + item.factoryOnlyCode
                              ? '_hov'
                              : '') +
                            '.png')
                      ...
                      <img
                        v-else
                        class="mr5"
                        src="@/assets/images/device/dl1.png"
                      />
                      电量{{
                        item.data.electQuantity
                          ? item.data.electQuantity + "%"
                          : "-"
                      }}
                    </div>
      ...
      <OrderList
        ...
        @clearMap="clearAllFence"
        @drawFencePlat="drawFencePlat"
      />
    </div>
    <div class="device-info-right">
      <div class="device-info-map-info ly-flex ly-flex-pack-justify">
        <div>
          ...
          <div class="status-groud ly-flex">
            ...
            <p>
              <span class="g-pot g-color-success" />在线 ({{
                statisticsData.onlineNum ? statisticsData.onlineNum : 0
              }})
            </p>
            <p>
              <span class="g-pot g-color-error" />过期 ({{
                statisticsData.expireNum ? statisticsData.expireNum : 0
              }})
            </p>
            <p>
              <span class="g-pot g-color-gray" />离线 ({{
                statisticsData.offlineNum ? statisticsData.offlineNum : 0
              }})
            </p>
            <!-- <p><span class="g-pot g-color-success" />激活 ({{ statisticsData.activeNum ? statisticsData.activeNum : 0 }})</p> -->
            <p>
              <span class="g-pot g-color-light-gray" />未激活 ({{
                statisticsData.noActiveNum ? statisticsData.noActiveNum : 0
              }})
            </p>
            <p>
              <span class="g-pot g-color-blue" />已绑定 ({{
                statisticsData.bindNum ? statisticsData.bindNum : 0
              }})
            </p>
            <p>
              <span class="g-pot g-color-blue-light" />未绑定 ({{
                statisticsData.noBindNum ? statisticsData.noBindNum : 0
              }})
            </p>
          </div>
        </div>
        <div class="device-config-box ly-flex">
          ...
          <p style="line-height: 32px; margin-left: 6px; color: #606266">
            刷新
          </p>
          <div
            class="show-button"
            :class="{ active: isShowDeviceName }"
            @click="showDeviceName"
          >
            显示设备名称
          </div>
        </div>
      ...
      <MapBox
        ...
        :is-show-device-name="isShowDeviceName"
        class="device-info-map-box"
...
<script>
import {
  getConsoleDeviceList,
  getConsoleDeviceLocation,
  getConsoleDeviceStatistics,
  getAllMapping,
} from "@/api/assets/device.js";
...
export default {
  ...
  methods: {
      ...
      list.forEach((el) => {
        ...
        this.allMapping[el[0]].forEach((val) => {
          labelArr.push({
            ...
            context:
              val.field_form_type === 2
                ? Number(deviceLocationObj[val.field_enname]).toFixed(
                    val.field_dit
                  )
                : deviceLocationObj[val.field_enname],
          });
        });
    ...
    async handleTrackPlayback(row) {
      ...
      this.allMapping[row.typeCode].forEach((val) => {
        labelArr.push({
          ...
          context:
            val.field_form_type === 2
              ? Number(row.data[val.field_enname]).toFixed(val.field_dit)
              : row.data[val.field_enname],
        });
      });
    ...
    /** 计算离线时间 */
    computeOfflineTime(time) {
      var leftOutline = "";
      if (time) {
        const nowTime = +new Date();
        const endTime = +new Date(time);
        var Times = (nowTime - endTime) / 1000;
        const d = parseInt(Times / 60 / 60 / 24);
        const H = parseInt((Times / 60 / 60) % 24);
        const m = parseInt((Times / 60) % 60);
        leftOutline = d + "天" + H + "小时" + m + "分钟";
      }
      let content = "";
      if (leftOutline) {
        content = "离线时长：" + leftOutline;
      } else {
        content = "离线时长：无";
      }
      return content;
...
.own-device-scroll-box::-webkit-scrollbar-thumb {
  /*滚动条里面小方块*/
  border-radius: 6px;
