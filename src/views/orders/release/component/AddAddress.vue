<template>
<div>
    <div style="border:1px solid #ccc; padding:10px;" v-for="(address,addindex) in addressList_to" :key="addindex">
        <el-form :ref="`elForm${address.time}`" :model="address" :rules="rules" :size="formConfig.size" :label-width="formConfig.labelWidth" :label-position="formConfig.labelPosition">
            ...
            <el-form-item label="详细地址">
                <el-input v-model="address.detail" placeholder="请输入具体地址" clearable :style="{width: '100%'}"></el-input>
            </el-form-item>

            <el-form-item label="地址别名">
                <el-input v-model="address.addressAlias" clearable placeholder="优先展示, 最多输入10个字" :style="{width: '100%'}"></el-input>
            </el-form-item>
            ...
            <el-button type="primary" @click="_delAddress(address)" v-if="addressList_to.length > 1">删除地址</el-button>
        </el-form>
    </div>  
    <el-button class="fahuoBtn" v-if="isBigOdd && showBtn" style="marginTop:10px;" type="primary" size="mini" @click="_addAddress">{{`+ 追加${title}地址`}}</el-button>
</div>
